...
static struct func_test func_ptr[FUNC_NB] = {{.func_ptr = test_byte_to_binary,    .func_name = "test bytes to binary string"},
                                             ...
                                             {.func_ptr = test_filling_sl_list,   .func_name = "test filling linked list"},
                                             {.func_ptr = test_foreach_sl_list,   .func_name = "test forearch linked list"}};

uint8_t test_foreach_sl_list(void)
{
    char test[5][10] = {"AZERTYUIO", "PQSDFGHJK", "LMWXCVBN?", "azertyuio", "pqsdfghjk"};
    simple_linked_list *list = NULL;
    simple_linked_list *current_list = NULL;
    uint32_t idx = 0;
    for (uint16_t i = 0; i < 5; i++)
    {
        if (coa_simple_linked_list(&list, test[i]) == 0)
            return EXIT_FAILURE;
    }
    int retour = EXIT_SUCCESS;
    while(1)
    {
        char* response = (char*)foreach_simple_node(list, &current_list, &idx);
        if(response == NULL)
            break;
        else if(strncmp(test[idx-1], response, 10))
        {
            retour = EXIT_FAILURE;
            break;
        }
...
uint8_t test_filling_sl_list(void)
{
    ...
    for(uint16_t i = 0 ; i < 5 ; i++)
    {
        char* response = (char*)get_simple_node(list, i);
        if(strncmp(test[i], response, 10))
        {
            retour = EXIT_FAILURE;
            break;
        }
